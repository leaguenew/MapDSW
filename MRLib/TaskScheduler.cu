...
__device__ void emit_intermediate(Intermediate* inter, SMCache* Cache,
	...
	//if Cache mode then use Cache else insert into Mem_Alloc
	 Cache->insert(inter, Mem_Alloc, isFail);
}
...
__global__ void Mapreducer(void* global_data_d, unsigned int* input_offset_d,
	...
	__shared__ bool fail[DIM_BLOCK];
	...
	for (int i = threadID; i < Num_chunks; i += Num_threads) {
		//do the map job
		//the input_offset_d[0] is the first offset in the global memory, since it is only for job, so the offset should all minus input_offset_d[0]

		if (i < Num_inputs) {
			//bool success=true;

			map(global_data_d, input_offset_d[i] - input_offset_d[0],
					&Cache[gid], mem_alloc_d, fail[tid]);

			//if the SMCache is full, stop all the threads and then swap the SMCache out
			//and merge to the memory allocator. Then flush the SMCache, and map again
			if (domerge) {
			    __syncthreads();
			  //  if(bid==0)printf("merging!!!!!!\n");
				...
				if(fail[tid])map(global_data_d, input_offset_d[i] - input_offset_d[0],
						&Cache[gid], mem_alloc_d,fail[tid]);
 ...
 * 1.Slice the input data into pieces which is maintained as Jobs in a Job sequence
 * 2.while the Jobqueue is not empty, Pop out a job from the Job sequence and then do the Map job.
 * 3.
 */
void TaskScheduler::doMapReduce() {
	//Some assertions

	if (CACHE_BUCKETS <= MAX_REMAIN_BUCKETS_C) {
		cout << "Cache_Buckets: " << CACHE_BUCKETS
				<< " should be larger than Max_remain_buckets_c :"
				<< MAX_REMAIN_BUCKETS_C << endl;
		exit(1);
	}

	if (CACHE_POOL <= CACHE_BUCKETS) {
		cout << "Cache_Pool size: " << CACHE_POOL
				<< " should be larger than Cache_buckets: " << CACHE_BUCKETS
				<< " in order to use more buckets" << endl;
		//exit(1);
	}

	int block_per_sm = 47 * 1024 / (sizeof(SMCache) * CACHEGROUP);
	if (block_per_sm * mySpecs->dim_block > 1536) {
		block_per_sm = 1536 / mySpecs->dim_block;
	}
	DoLog("=====================================");
	DoLog("The max block per SM is: %d",block_per_sm);
	...
	while (!JobQueue.empty()) {
		DoLog("A Job has been initiated...");
